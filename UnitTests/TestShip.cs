...
namespace Vsite.Oom.Battleship.Model.UnitTests {
    ...
    public class TestShip {
        ...
        [TestMethod]
        public void HitReturnsMissedIfSquareProvidedDoesntBelongToShip() {
            List<Square> squares = new List<Square> { new Square(1, 2), new Square(1, 3), new Square(1, 4) };
            var ship = new Ship(squares);
            Assert.AreEqual(HitResult.Missed, ship.Hit(new Square(5, 4)));
        }
        [TestMethod]
        public void HitReturnsMissedIfSquareProvidedBelongsToShip() {
            List<Square> squares = new List<Square> { new Square(1, 2), new Square(1, 3), new Square(1, 4) };
            var ship = new Ship(squares);
            Assert.AreEqual(HitResult.Hit, ship.Hit(new Square(1, 2)));
        }
        [TestMethod]
        public void HitReturnsMissedIfSquareProvidedIsLastSquareBelongingToShip() {
            List<Square> squares = new List<Square> { new Square(1, 2), new Square(1, 3), new Square(1, 4) };
            var ship = new Ship(squares);
            Assert.AreEqual(HitResult.Hit, ship.Hit(new Square(1, 2)));
